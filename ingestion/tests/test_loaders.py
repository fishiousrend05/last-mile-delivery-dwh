...
start_run, end_run, get_running_since — vì unit test không có Postgres
thật; kiểm tra ĐÚNG LUỒNG bằng Manual E2E test riêng, không phải ở đây).

Lưu ý: postgres_loader.load_dataframe() chặn concurrent load bằng
get_running_since() (trả về started_at nếu run gần nhất đang 'running',
None nếu không) + STALE_RUNNING_THRESHOLD_MINUTES để tự phục hồi khi lock
bị treo do crash — KHÔNG dùng get_last_run_status() (chỉ trả về status
string, không đủ thông tin để tính tuổi lock).
"""
...
class TestPostgresLoaderControlFlow:
    ...
    def test_concurrent_load_blocked_when_lock_is_fresh(self, monkeypatch):
        """Run gần nhất đang 'running' và mới bắt đầu (< threshold) -> phải chặn."""
        start_run_calls = []
        monkeypatch.setattr(postgres_loader, "init_metadata_table", lambda: None)
        monkeypatch.setattr(
            postgres_loader,
            "get_running_since",
            lambda s, t: datetime.now(timezone.utc),  # mới bắt đầu ngay bây giờ
        )
        monkeypatch.setattr(
    ...
    def test_stale_lock_is_recovered_not_blocked(self, monkeypatch):
        """Run 'running' cũ hơn STALE_RUNNING_THRESHOLD_MINUTES -> coi là lock
        treo do crash, KHÔNG chặn, cho phép load tiếp tục bình thường."""
        to_sql_calls = {}

        def fake_to_sql(self, name, con, schema, if_exists, index, chunksize, method):
            to_sql_calls["called"] = True

        stale_since = datetime.now(timezone.utc) - timedelta(
            minutes=postgres_loader.STALE_RUNNING_THRESHOLD_MINUTES + 1
        )

        monkeypatch.setattr(pd.DataFrame, "to_sql", fake_to_sql)
        monkeypatch.setattr(postgres_loader, "get_engine", lambda: _FakeEngine())
        monkeypatch.setattr(postgres_loader, "init_metadata_table", lambda: None)
        monkeypatch.setattr(postgres_loader, "get_running_since", lambda s, t: stale_since)
        monkeypatch.setattr(postgres_loader, "start_run", lambda s, t, m: "run-stale")
        monkeypatch.setattr(postgres_loader, "end_run", lambda *a, **kw: None)

        df = pd.DataFrame({"a": [1]})
        row_count = postgres_loader.load_dataframe(df, "src", "tbl", "full_refresh")

        assert row_count == 1
        assert to_sql_calls.get("called") is True  # load vẫn chạy, không bị chặn
    ...
    def test_success_path_calls_end_run_success_and_returns_row_count(self, monkeypatch):
        ...
        monkeypatch.setattr(postgres_loader, "get_engine", lambda: _FakeEngine())
        monkeypatch.setattr(postgres_loader, "init_metadata_table", lambda: None)
        monkeypatch.setattr(postgres_loader, "get_running_since", lambda s, t: None)
