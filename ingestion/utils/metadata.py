...
def get_running_since(source_name: str, table_name: str) -> datetime | None:
    """
    Nếu lần chạy gần nhất của (source, table) đang ở trạng thái 'running',
    trả về started_at của nó — None nếu không có run nào đang chạy.

    Dùng để phân biệt 2 tình huống trông giống hệt nhau qua status='running':
        1. Một tiến trình KHÁC thật sự đang chạy ngay lúc này (nên chặn).
        2. Một tiến trình cũ đã CRASH (Ctrl+C, kill -9, mất điện...) và không
           kịp gọi end_run() — status='running' bị TREO VĨNH VIỄN dù chẳng
           còn tiến trình nào chạy cả.

    Không thể phân biệt 2 case này chỉ bằng status — cần thêm "tuổi" của
    run đó (xem STALE_RUNNING_THRESHOLD_MINUTES trong postgres_loader.py).
    """
    with get_session() as session:
        result = session.execute(
            ingestion_audit_log.select()
